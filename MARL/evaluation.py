...
def main():
  ...
  for _ in range(len(save_name_list)):
    ...
    for i in range(ego_seed):
      ...
      ego_checkpoint.append([os.path.join(temp_dir, gen[_], checkpoint_list[_]) for _ in range(len(gen))])
    
    print(ego_checkpoint) # ego_checkpoint is a list of size #seed x #gen
    ...
    print(opponent_checkpoint) # opponent_checkpoint is a list of size 4
    ...
    for seed in range(len(ego_checkpoint)):
      for gen in range(len(ego_checkpoint[seed])):
        for opponent in range(len(opponent_checkpoint)):
          ...
          ego_trainer.restore(ego_checkpoint[seed][gen])
          ...
          for ep in range(episode_num):
            ...
            print('-'*50)
            print(f'Seed {seed:d}: Ego {gen:d}xOpp {opponent:d}\tEpisode {ep:d}\tEgo reward: {ep_rewards[0]:.3f}\tOpponent reward: {ep_rewards[1]:.3f}')

            # save rewards
            rewards[seed, gen, opponent, ep] = ep_rewards[0]
